...
jmp 0x07C0:START    ; CS 세그먼트 레지스터에 0x07C0을 복사하면서, START 레이블로 이동
                    ...
                    ; 리얼모드이므로 물리 주소 변환방식은 세그먼트 레지스터 * 0x10 + 범용 레지스터
                    ; 따라서, 코드가 시작하는 0x7C00을 가리키려면 0x7C00 / 0x10 값을 
                    ; 세그먼트 레지스터 값에 넣어줘야 하기 때문에 0x07C0 값을 넣음
                    ; jmp 코드를 쓰면 IP 레지스터 값이 변한다고 알고 있는데 세그먼트 레지스터 값도 변하나보다
...
TOTALSECTORCOUNT:   dw  0x02    ; 부트 로더를 제외한 MINT64 OS 이미지의 크기
                                ...
                                ; 3.5인치 플로피 디스크에 저장할 예정이기 때문에
                                ; 섹터 수(18) * 트랙 수(80) * 헤드 수(2) = 총 2880섹터
                                ; 까지 저장 가능하지만 주석에 써져있는 최대 1152 섹터까지 가능
                                ; 하다는 이야기는 무슨 말인지 잘 모르겠다
                                ; 일단 MINT64 OS 이미지를 1024 섹터(512kb) 크기로 개발할
                                ; 예정이라고 생각해야겠다
                                
                                ; 0xA0000 이후 영역은 비디오 메모리로 사용되기 때문에
                                ; 리얼모드에서 바이오스 서비스를 최대한 활용하려다 보니
                                ; 플로피 디스크 크기가 크더라도 0x10000 부터 시작되는
                                ; OS크기가 제한될 수 밖에 없음 (p.164)

                                ; 원래 값은 1024 였으나 6장에서 1로 수정
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
START:
  mov ax, 0x07C0  ; 부트 로더의 시작 어드레스(0x07C00)를 세그먼트 레지스터 값으로 변환
  mov ds, ax      ; DS 세그먼트 레지스터에 설정
  mov ax, 0xB800  ; 비디오 메모리의 시작 어드레스(0xB8000)를 세그먼트 레지스터 값으로 변환
...
.SCREENCLEARLOOP:                 ; 화면을 지우는 루프
                                  ...
                                  ; 추가로 작성한 코드를 보면 함수는 .을 붙이지 않았음
                                  ; 루프코드 라벨에 .을 붙인 이유는 함수나 변수가 아니라
                                  ; 단순 로직이라는 점을 알리기 위해서 인것 같음
...
READDATA:                           ; 디스크를 읽는 코드의 시작
  ...
  ; 한 섹터를 읽었으므로 섹터 번호를 증가시키고 마지막 섹터(36)까지 읽었는지 판단
  ...
  cmp al, 37                          ; 증가시킨 섹터 번호를 37과 비교
                                      ...
                                      ; 원본 소스에는 섹터 번호를 19와 비교하지만
                                      ; qemu 에뮬레이터의 플로피디스크가 
                                      ; 1.44 MB FD에서 2.88 MB FD로 변경되면서
                                      ; FDRIVE_DISK_144   1.44 MB disk 
                                      ; FDRIVE_DISK_288   2.88 MB disk
                                      ; 트랙 당 섹터 개수가 18개에서 36개로 변경되었음
                                      ; 따라서, 섹터 마지막을 판단하기 위해 37과 비교                

  jl READDATA                         ; 섹터 번호가 37 미만이라면 READDATA로 이동
  
  ; 마지막 섹터까지 읽었으면(섹터 번호가 37이면) 헤드를 토글(0->1, 1->0)하고, 
  ; 섹터 번호를 1로 설정
...
READEND:
  ...
  push 20                         ; 화면 X 좌표(20)를 스택에 삽입 
                                  ...
                                  ; IMAGELOADINGMESSAGE 길이가 19이기 때문에 20부터 시작
